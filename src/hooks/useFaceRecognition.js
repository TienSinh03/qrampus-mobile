...
if (ONNX_ENABLED) {
  try {
    ...
    console.warn('[FaceRecognition] ONNX Runtime is unavailable:', e.message);
  }
}
...
function preprocessFace(pixels) {
  ...
  for (let i = 0; i < 112 * 112; i++) {
    data[i] = (pixels[i * 3] - 127.5) / 127.5;
    data[i + 112 * 112] = (pixels[i * 3 + 1] - 127.5) / 127.5;
    data[i + 2 * 112 * 112] = (pixels[i * 3 + 2] - 127.5) / 127.5;
  }
...
export function useFaceRecognition() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        console.log('[FaceRecognition] Loading local ONNX model...');
        const t0 = Date.now();
        ...
        console.log(`[FaceRecognition] Model loaded (${Date.now() - t0}ms)`);
      } catch (e) {
  ...
  const getEmbedding = async (facePixels) => {
    if (!ort) throw new Error('ONNX Runtime is temporarily disabled');
    if (!sessionRef.current) throw new Error('Model has not finished loading');

    console.log(`[FaceRecognition] getEmbedding input size: ${facePixels.length}`);
    const t0 = Date.now();
    const tPre = Date.now();
    ...
    console.log(
      `[FaceRecognition] inference: ${Date.now() - tInfer}ms | total: ${Date.now() - t0}ms | dim: ${embedding.length}`
    );
    return embedding;
  };

  const compare = async (face1, face2, threshold = 0.3) => {
    console.log('[FaceRecognition] compare start');
    const t0 = Date.now();
    ...
    console.log(
      `[FaceRecognition] compare result: emb1=${t1 - t0}ms emb2=${t2 - t1}ms cosine=${Date.now() - t2}ms total=${Date.now() - t0}ms score=${score.toFixed(4)}`
    );
