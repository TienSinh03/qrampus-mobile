...
const NotificationItem = ({ 
  ...
  const { icon, iconColor } = getNotificationTypeConfig(notification.type);
  ...
  return (
    <TouchableOpacity
      ...
      className={`mx-4 mb-3 rounded-xl overflow-hidden ${
        notification.is_read ? 'bg-white' : configRole.unreadBg
      } ${getPriorityBorder(notification.priority, userRole)}`}
      style={{
        shadowColor: '#000',
        ...
        <View className="flex-1">
          <View className="flex-row items-start justify-between mb-1">
            <Text className={`text-base font-bold flex-1 ${
              notification.is_read ? 'text-gray-900' : configRole.unreadText
            }`}>
              {notification.title}
            </Text>
